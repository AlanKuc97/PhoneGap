class MainMenu extends Phaser.Scene{
	...
	create(){
		this.logo = this.add.sprite(config.width/2,config.height/2 - 170,'logo').setOrigin(0.5);
	...
	update(){
		if(this.start.scaleX > 0.55 && this.scaleDirection > 0){
			this.scaleDirection = -this.scaleDirection;
		}else if(this.start.scaleX < 0.5 && this.scaleDirection < 0){
			this.scaleDirection = -this.scaleDirection;
		}
