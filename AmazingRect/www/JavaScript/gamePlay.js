class GamePlay extends Phaser.Scene{
	...
	create(){
		...
		for( let i = 0; i < 6; i++){
			if( i < 2){
				this.smallObstacles.push(this.add.rectangle(
					Math.floor(Math.random() * (config.width -16 - 16) + 16),
					Math.floor(Math.random() * ((this.firstRect[0].y - 50 - this.firstRect[0].height) - this.secondRect[0].y) 
																							   + this.secondRect[0].y
																							   + this.firstRect[0].height),
					16,16,0x5ca8F8
				));
			}else if( i < 4){
				this.smallObstacles.push(this.add.rectangle(
					Math.floor(Math.random() * (config.width -16 - 16) + 16),
					Math.floor(Math.random() * ((this.secondRect[0].y  - 50 - this.secondRect[0].height) - this.thirdRect[0].y)
																								  + this.thirdRect[0].y
																								  + this.secondRect[0].height),
					16,16,0x1F841A
				));
			}else{
				this.smallObstacles.push(this.add.rectangle(
					Math.floor(Math.random() * (config.width -16 - 16) + 16),
					Math.floor(Math.random() * ((this.thirdRect[0].y  - 50 - this.thirdRect[0].height) - (this.thirdRect[0].y - 300))
																								  + (this.thirdRect[0].y - 300)
																								  + this.thirdRect[0].height),
					16,16,0xfca858
				));
			}
			this.physics.add.existing(this.smallObstacles[i]);
		}
		//Graphics config
	...
	rectangleLoop(){
		if(this.firstRect[0].y > config.height){
			this.firstRect[2] = this.colorArr[this.colorArr.length * Math.random() | 0];
			this.firstRect[0].y = this.thirdRect[0].y - 400;
			this.firstRect[1].y = this.thirdRect[0].y - 400;
			this.firstRect[0].x = Math.floor(Math.random() * (this.MAX - this.MIN) + this.MIN);
			this.firstRect[1].x = this.firstRect[0].x + this.firstRect[0].width + this.spaceBetween;
			this.firstScored = 0;
			this.setRandomObstaclesPosition(4,6);
		}else if(this.secondRect[0].y > config.height){
			this.secondRect[2] = this.colorArr[this.colorArr.length * Math.random() | 0];
			this.secondRect[0].y = this.firstRect[0].y - 400;
			this.secondRect[1].y = this.firstRect[0].y - 400;
			this.secondRect[0].x = Math.floor(Math.random() * (this.MAX - this.MIN) + this.MIN);
			this.secondRect[1].x = this.secondRect[0].x + this.secondRect[0].width + this.spaceBetween;
			this.secondScored = 0;
			this.setRandomObstaclesPosition(0,2);
		}else if(this.thirdRect[0].y > config.height){
			this.thirdRect[2] = this.colorArr[this.colorArr.length * Math.random() | 0];
			this.thirdRect[0].y = this.secondRect[0].y - 400;
			this.thirdRect[1].y = this.secondRect[0].y - 400;
			this.thirdRect[0].x = Math.floor(Math.random() * (this.MAX - this.MIN) + this.MIN);
			this.thirdRect[1].x = this.thirdRect[0].x + this.thirdRect[0].width + this.spaceBetween;
			this.thirdScored = 0;
			this.setRandomObstaclesPosition(2,4);
		}
	}
	setRandomObstaclesPosition(startIndex, endIndex){
		for( let i = startIndex; i < endIndex; i++){
			if( i < 2){
				this.smallObstacles[i].x = Math.floor(Math.random() * (config.width -16 - 16) + 16);
				this.smallObstacles[i].y = Math.floor(Math.random() * 
											(this.firstRect[0].y - 50 - this.firstRect[0].height - this.secondRect[0].y)
																							+ this.secondRect[0].y
																							+ this.firstRect[0].height);
			}else if( i< 4){
				this.smallObstacles[i].x = Math.floor(Math.random() * (config.width -16 - 16) + 16)
				this.smallObstacles[i].y = Math.floor(Math.random() * 
											(this.secondRect[0].y - 50 - this.secondRect[0].height - this.thirdRect[0].y)
																							  + this.thirdRect[0].y
																							  + this.secondRect[0].height);
			}else{
				this.smallObstacles[i].x = Math.floor(Math.random() * (config.width -16 - 16) + 16);
				this.smallObstacles[i].y = Math.floor(Math.random() * 
											(this.thirdRect[0].y  - 50 - this.thirdRect[0].height - (this.thirdRect[0].y - 300))
																							   + (this.thirdRect[0].y - 300)
																							   + this.thirdRect[0].height);
			}
		}
